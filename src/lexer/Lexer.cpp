...
void Lexer::logLexerError(std::string errorMessage) {
  std::cerr << colors["red"] << "Error in Lexer: " << errorMessage
            << " on line: " << mCurrentLine;
}
...
std::string Lexer::mUnescape(const std::string &content) {
  std::string result;
  result.reserve(content.size());  // reserve approximate space
  ...
  for (size_t i = 0; i < content.size(); ++i) {
    if (content[i] == '\\' && i + 1 < content.size()) {
      ...
      i++;  // skip the next character since it was part of escape sequence
    } else {
...
Token Lexer::mMakeStringLiteralToken(std::size_t line, std::size_t col,
  ...
  // TODO unescape
  return Token{line, col, TokenType::StringLiteral, mUnescape(stringContent)};
}
...
std::vector<Token> Lexer::parseSource() {
  ...
  while (isUnderEnd()) {
    ...
    if (mIsWordBeginChar(peek())) {
      ...
      } else if (mStrSource.length() > mCurrentPos &&
                 mStrSource.at(mCurrentPos + 1) == '*') {
      ...
      mAddToken({line, col, TokenType::CharLiteral, mUnescape(mStringLiteralCharacter())});
      if (!consume('\''))
        logLexerError("improper Character Literal");
    } else if (consume('-')) {
      if (consume('>'))
