    ...
    unique_ptr<Expressions::Expression> Parser::mPrimaryExpression() {
        ...
        if (match(TokenType::Identifier)) {
            ...
            if (consume(TokenType::Increment))
                expr = make_unique<Expressions::Unary>(std::move(expr), UnaryOperatorType::PostIncrement);
            if (consume(TokenType::Decrement))
                expr = make_unique<Expressions::Unary>(std::move(expr), UnaryOperatorType::PostDecrement);
            return expr;
        }
