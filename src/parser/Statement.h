...
public:
  IfStatement(std::unique_ptr<Expressions::Expression> cond,
...
public:
  ElseStatement(std::unique_ptr<Statement> stmt)
...
public:
  explicit BlockStatement(vector<std::unique_ptr<Statement>> nodes)
...
public:
  ReturnStatement(std::unique_ptr<Expressions::Expression> expr)
      : expression{std::move(expr)} {};
  std::unique_ptr<Expressions::Expression> expression;
...
public:
  VariableDefinitionStatement(
      std::string varName,
      std::unique_ptr<TypeExpressions::TypeExpression> varType,
      std::unique_ptr<Expressions::Expression> varValue)
...
public:
  ...
  Parameter(std::unique_ptr<TypeExpressions::TypeExpression> type,
            std::string name)
...
public:
  FunctionDefinition(std::unique_ptr<TypeExpressions::TypeExpression> ret,
      ...
      : functionName{std::move(name)}, parameters{std::move(params)},
        statements(std::move(code)), returnType{std::move(ret)} {};
